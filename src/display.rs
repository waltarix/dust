...
fn pad_or_trim_filename(node: &DisplayNode, indent: &str, display_data: &DisplayData) -> (String, String) {
    ...
    (maybe_trim_filename(name, display_data, ""), padding)
}

fn maybe_trim_filename(name_in: String, display_data: &DisplayData, indent: &str) -> String {
    if UnicodeWidthStr::width(&*name_in) > display_data.longest_string_length {
        let mut width_counter = 0;
        let indent_width = max(indent.chars().count() as i32 - 7, 0) as usize;
        let max_width = display_data.longest_string_length - 2 - indent_width;
        let name = name_in
            .chars()
            .filter(|c| {
                if width_counter > max_width {
                    ...
                    width_counter += UnicodeWidthChar::width(*c).unwrap_or(0);
                    width_counter <= max_width
                }
            })
            .collect::<String>();
        name + ".."
...
fn format_percents(bar_chart_in: &str, percent_size_str: String, is_colors_on: bool) -> String {
    let bar_chart = if is_colors_on {
        Fixed(228).paint(bar_chart_in).to_string()
    } else {
        bar_chart_in.into()
    };
...
fn get_name_percent(
    ...
    if !bar_chart.is_empty() {
        ...
        let percents = format_percents(bar_chart, percent_size_str, display_data.colors_on);
        ...
        let name = maybe_trim_filename(n, display_data, indent);
        ("".into(), name, "".into())
    }
}
